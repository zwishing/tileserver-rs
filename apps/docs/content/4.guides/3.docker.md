...
Official images are published to GitHub Container Registry in three tiers:

| Tag suffix | Description | Built with | Target hardware |
| ---------- | ----------- | ---------- | --------------- |
| *(none)* | Default: embedded frontend + `geoparquet` + `stac` features | `-C target-cpu=x86-64-v3` (amd64) / `neoverse-n1` (arm64) | Any Intel/AMD since 2013, Graviton2+ |
| `-headless` | No embedded frontend (server-only) | Same baseline CPU | Same |
| `-fast` | Embedded frontend + `geoparquet` + `stac` | `-C target-cpu=x86-64-v4` (AVX-512) / `neoverse-v1` | Post-2017 Intel/AMD server CPUs, Graviton3/3E, Azure Ampere Altra Max |

Pick `-fast` only when you control the hardware. On older/mixed fleets, the default tier is safer (pulling `:X.Y.Z-fast` onto an unsupported CPU will fault with SIGILL at startup).

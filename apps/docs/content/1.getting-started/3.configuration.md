...
tileserver-rs includes an in-process tile cache backed by [moka](https://github.com/moka-rs/moka). When enabled, tiles are cached in memory with byte-weighted eviction and per-entry TTL. This eliminates repeated disk I/O or network calls for frequently requested tiles.

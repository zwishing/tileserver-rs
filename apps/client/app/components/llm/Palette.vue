<script setup lang="ts">
  ...
  const {
    messages, isLoading, engineStatus, loadProgress, loadStageText,
    selectedModel, availableModels, engineError, input,
...
<template>
  ...
  <Transition enter-active-class="duration-200 ease-out" enter-from-class="translate-y-4 scale-95 opacity-0" leave-active-class="duration-150 ease-in" leave-to-class="translate-y-4 scale-95 opacity-0">
    <div
      v-if="open"
      ref="panelRef"
    ...
    >
      ...
      <div class="flex items-center justify-between border-b border-border/50 bg-muted/30 px-4 py-2">
        ...
        <div class="flex items-center gap-2">
          <LlmModelSelect :models="availableModels" :selected-id="selectedModel.id" :disabled="engineStatus === 'loading'" class="w-40" @select="selectModel" />
          <button class="flex size-7 items-center justify-center text-muted-foreground transition-colors hover:text-foreground" @click="close">
            <X class="size-3.5" />
          </button>
        </div>
      </div>
      ...
      <!-- Engine error -->
      <div v-if="engineError" class="border-b border-destructive/20 bg-destructive/5 px-4 py-2">
      ...
      <div v-else class="min-h-0 flex-1 overflow-y-auto">
        <div v-if="messages.length === 0" class="flex flex-col gap-3 p-4">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            >
              <component :is="getIconComponent(s.icon)" class="size-3.5 text-muted-foreground transition-colors group-hover:text-primary" />
              <span class="text-[11px] font-medium leading-snug">{{ s.title }}</span>
            </button>
          </div>
        </div>

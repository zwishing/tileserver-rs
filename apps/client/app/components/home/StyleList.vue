    ...
    <Collapsible :open="isOpen" @update:open="$emit('update:isOpen', $event)">
      <Card class="overflow-hidden rounded-xl border-border/50 bg-card/50 backdrop-blur-sm">
        <CollapsibleTrigger class="flex w-full items-center gap-3 p-4 transition-colors hover:bg-muted/30">
          ...
          <Badge variant="secondary" class="ml-auto rounded-lg">{{ styles.length }}</Badge>
        </CollapsibleTrigger>
        ...
        <CollapsibleContent>
          ...
          <div class="p-4">
            ...
            <div v-else-if="!hasStyles" class="py-12 text-center">
              ...
              <p class="mt-1 text-sm text-muted-foreground">Add styles to your config.toml</p>
            </div>

            <!-- No results -->
            <div v-else-if="styles.length === 0" class="py-12 text-center text-muted-foreground">
